...
- Address by designing sampling and weighting schemes that correct for overlapping influence.

NUMBER OF CONCURRENT LABELS
- Two labels yi and yj are concurrent at t when both are a function of at least one common return.
- The overlap does not need to be perfect, in the sense of both labels spanning the same time interval.

AVERAGE UNIQUENESS OF A LABEL
- Uniqueness = non-overlap
- This procedure allows us to assign a uniqueness score between 0 and 1 for each observed feature.

BAGGING CLASSIFIERS AND UNIQUENESS
- Redundancy of draws makes the bootstrap inefficient.
-  Sequential Bootstrap: Draws are made according to a changing probability that controls for redundancy.

RETURN ATTRIBUTION
- Bootstrap samples closer to IID used to weight samples for purpose of training an ML algorithm.
- We wish to weight an observation as a function of the absolute log returns that can be attributed uniquely to it.
- Will not work if there's a neutral case.
- Generally advise you to drop “neutral” cases.

TIME DECAY
- We would typically like sample weights to decay as new observations arrive.
- The final weight has no decay.

CLASS WEIGHTS
- Class weights are weights that correct for underrepresented labels.
- This is particularly critical in classification problems where the most important classes have rare occurrences. i.e. flash crash.


## Fractionally Differentiated Features
- As a consequence of arbitrage forces, financial series exhibit low signal-to-noise ratios.
- Current techniques lead to false discoveries.
- Non-stationary time series = presence of memory(history of levels that shift mean)
- Stationarity provides inferential analyses.
- Goal is to retain some of the memory. We want to when a process has drifted. 
- What is the minimum amount of differentiation that makes a price series stationary while preserving as much memory as possible? 
- Framework: stationary series where not all memory is erased.

STATIONARITY VS. MEMORY DILEMMA
- Stationarity transforms further reduce signal by removing memory. 
- There is a trade-off between stationarity and memory.

LITERATURE REVIEW
- Fractional differentiation applied to the predictive time series analysis has been around for awhile but not applied to financial series. 
